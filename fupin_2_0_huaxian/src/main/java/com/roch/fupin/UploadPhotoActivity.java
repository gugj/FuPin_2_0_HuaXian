...
public class UploadPhotoActivity extends MainBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//初始化toolbar，并设置上传照片按钮的点击监听
		initToolbar();
		//通过intent获取传送的数据，并设置ListView的适配器
		initDate();
	}
	...
	private void initDate() {
		...
		if (bundle != null) {
			...
			lists = new ArrayList<com.roch.fupin.entity.UploadPhoto>();
	...
	protected void UploadPhoto(final int i) {
		...
		System.out.println("上传图片时householderid="+id+"&file="+new File(lists.get(i).getPath())+"&url="+url);
		...
		httpUtils.send(HttpRequest.HttpMethod.POST, url, rp, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {
				...
				if (list != null) {
					if (list.getSuccess()) { //上传照片服务器返回数据---成功
							...
							if (adapter.getCount() > 0) {
								...
								if(StringUtil.isNotEmpty(shangchuan_type)){
									if("hu".equals(shangchuan_type)){
										...
										intent.setAction("photo_shangchuan"); // photo_shangchuan_hu
										sendBroadcast(intent);
									}else if("cun".equals(shangchuan_type)){
										Intent intent=new Intent();
										intent.setAction("photo_shangchuan"); //photo_shangchuan_cun
										sendBroadcast(intent);
									}
								}
